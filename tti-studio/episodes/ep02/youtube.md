...
- The API as the waiter — one backend serving many apps (iOS, Android, web)
- CSR vs SSR — who actually draws the page, and the trade-off
- IP addresses as a server's "phone number" (IPv4 vs IPv6)
- DNS as the internet's phonebook — name → number
- The DNS relay: device → resolver → root → TLD → authoritative
- DNS caching and TTL — the speed-vs-freshness trade-off
- HTTP as the shared language — request and response
- Anatomy of a request: method, path, headers, body — and a URL broken down
...
- HTTPS as a sealed envelope — what the padlock actually means (and what it doesn't)
...
0:00 Recap & the missing piece
0:33 Tonight's two questions
0:52 The restaurant (the spine analogy)
1:23 The client — your phone, the dining room
1:52 The server — the kitchen
2:19 Where your data actually lives
2:46 The golden rule: never trust the client
3:15 The API — the waiter
3:50 CSR vs SSR — who draws the page?
4:28 Billions of computers, one name
4:49 IP address — the server's phone number
5:13 DNS — the internet's phonebook
5:34 The DNS relay (resolver → root → TLD → authoritative)
6:12 DNS cache + TTL — the trade-off
6:47 HTTP — the shared language
7:14 Anatomy of a request
7:43 GET vs POST — read vs write
8:23 Cookies — your ID badge
8:46 Status codes — 200, 301, 404, 500
9:19 The S in HTTPS — sealed envelope
9:52 The whole journey, end to end
10:20 Recap + what's next
